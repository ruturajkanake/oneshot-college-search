    ...
    return (
        <div className='table-data'>
            ...
            <Row justify='space-between'>
                <Col>
                    <i class='fas fa-map-marker-alt' style={{fontSize:'17px'}}></i>
                    <Text style={{fontSize:'17px', padding: '5px'}}>{college.city}, {college.state}, {college.country}</Text>
                </Col>
                <Col>
                <Text style={{fontSize:'17px', padding: '5px'}}>Year Founded: {college.year}</Text>
                </Col>
            </Row>
            ...
            <div>
                {college.courses.map((course, i) => {
                    return <Tag  style={{fontSize: '15px', padding: '5px', marginBottom: '5px'}} color={randomTagColor()}>{course}</Tag>
                })}
            </div>
            <Row justify='start' style={{marginTop: 50}}>
                <Col style={{padding: '6px'}}>
                    <Text style={{fontSize:'18px', padding: '50px 10px 0 0'}}>Number of Students: {college.no_of_students}</Text>
                </Col>
                <Col>
                    <Link to={`/student/${props.match.params.collegeId}`}>
                        <Button size='large' danger>Students List</Button>
                    </Link>
            ...
            <Row justify='space-around'>
                {similarColleges && similarColleges.map((similarCollege) => {
                    return <Link to={`/college/${similarCollege.id}`} onClick={window.scrollTo(0, 0)}><Card hoverable title={similarCollege.name} style={{width:300, marginBottom: 10}}>
